...
- ✅ **Sistema elemental**: 5 elementos (physical/fire/ice/electric/poison) como affix de arma, com **2 opções de arma por elemento** (10 armas elementais + 2 físicas) e 4 escudos com variação de defesa.
- ✅ **Status effects elementais** (novo em v0.0.7): golpes elementais (jogador ↔ inimigo) podem aplicar burn (DoT), slow→freeze (stacking até incapacitação), shock (amplifica o próximo golpe, pode espalhar e paralisar) e poison (stacking infinito). Visível na HUD do jogador.
- ✅ **7 tipos de inimigos**, todos com `skillLootTable`, distribuídos por **pool ponderado por profundidade** (andares 1–2 só dificuldade baixa; 3–4 introduzem os elementais; andar 5 prioriza os mais difíceis).
...
- 🚧 Fora de escopo nesta versão: confronto de chefe roteirizado no andar final, descida além do andar 5, resistências/fraquezas elementais (defesa continua puramente numérica), economia, facções, vila/fazenda ou ranking.
- ⚠️ **Achado conhecido, não corrigido nesta versão** (fora do escopo dos arquivos v0.0.7 — reportado para triagem): em raríssimos seeds (5 em 500 testados) uma entrada de dungeon do overworld pode coincidir de posição com uma entidade spawnada. Bug pré-existente em `spawnEnemies()`/geração de entradas do overworld (v0.0.6), não introduzido por esta versão. Ver seção de testes abaixo.

## Changelog

### v0.0.7
- **Status effects elementais** (`js/core/state.js`, `STATUS_CONFIG`): motor genérico (`applyStatusEffect`/`processStatusEffects`/`getStatusEffect`) que funciona tanto para `STATE.player` quanto para entidades do mapa:
  - **burn** (fire): 40% de chance por golpe. DoT por 3 turnos, dano/turno = metade do `elementDamage` do golpe (mín. 1). Reaplicar renova a duração.
  - **slow** (ice): 30% de chance. Empilha até 4 stacks (cada aplicação soma 1 e renova a duração para 4 turnos); ao atingir 4 stacks aplica **freeze** (2 turnos totalmente incapacitado) e reseta os stacks.
  - **shock** (electric): garantido em todo golpe elétrico. Amplifica em 1.5x o próximo golpe sofrido pelo alvo e é consumido nesse golpe (ou expira em 5 turnos sem ser consumido). Adicionalmente: 50% de chance de espalhar para inimigos próximos (raio 2, só quando o jogador ataca) e 20% de chance de aplicar **paralyze** (pula 1 turno) diretamente.
  - **poison**: 75% de chance. Cada aplicação soma 5 stacks de uma vez; cada stack causa 2 de dano/turno. Empilha infinitamente, sem expiração natural.
  - Bidirecional: inimigos com `elementType` definido aplicam os mesmos efeitos no jogador. Status ativo é exibido na HUD (`js/ui/hud.js`).
  - `engine.js` (fora da lista original de arquivos, necessidade emergente documentada em `[v0.0.7-D]`): `movePlayer`/`playerWait` bloqueiam a ação do jogador enquanto `freeze`/`paralyze` estiverem ativos, ainda consumindo o turno (senão o efeito nunca decrementaria); `startNewGame()` agora reseta `player.statusEffects`.
- **4 novos inimigos** (`data/enemies.json`), todos com `elementType`/`elementDamage`/`skillLootTable`: Salamandra de Lava (fire, intermediária), Elemental de Gelo (ice, tanque puro — def 15), Esporo Tóxico (poison, HP muito baixo, perigoso em DoT), Condutor de Relâmpago (electric, glass cannon).
- **6 novos itens** (`data/items.json`): 4 armas (2ª opção por elemento fire/ice/electric/poison, com tipos variados — machado, lança, arco, garra) + 2 escudos (`ember_ward`, `storm_plate`). Todo elemento agora tem 2 opções de arma.
- **Pool de inimigos ponderado por profundidade** (`js/world/mapGen.js`, `ENEMY_POOL_BY_DEPTH`): substitui o roll uniforme por tabela de pesos `{id, weight}` por faixa de andar. Andares 1–2 só geram os 3 inimigos de baixa dificuldade; 3–4 introduzem os elementais (steam_golem raro); andar 5 prioriza ice/electric/steam_golem. Salas opcionais de risco/recompensa também respeitam o gate de profundidade.
...
**Novo inimigo:** edite `data/enemies.json`. Copie um inimigo existente como modelo (`hp`, `attack`, `defense`, `xp`, `char`, `color`, `ai`, `lootTable`, `skillLootTable` opcional; `elementType`/`elementDamage` se o inimigo tiver ataque elemental). Para aparecer no **overworld**, adicione o `id` à lista `enemyTypes`/`weights` dentro de `spawnEnemies()` (`js/world/mapGen.js`). Para aparecer na **dungeon**, adicione o `id` (com peso) à faixa de profundidade apropriada em `ENEMY_POOL_BY_DEPTH` (mesmo arquivo) — sem isso, um inimigo novo só existe em `defs.enemies` mas nunca é sorteado em nenhum andar.

**Novo item:** edite `data/items.json`. Defina `type` (`consumable`, `weapon`, `armor` ou `material`) e os campos correspondentes (`effect`/`value` para consumíveis, `attackBonus`/`elementDamage`/`elementType` para armas, `defenseBonus` para armaduras/escudos). Itens com `attackBonus`/`defenseBonus` entram automaticamente no pool premium de drop da dungeon (`spawnDungeonItems()`); para aparecer no chão do **overworld**, adicione-o também ao `pool` de `spawnItems()` (`js/world/mapGen.js`) — por design, armas/escudos elementais só dropam na dungeon (overworld só tem equipamento físico básico).
