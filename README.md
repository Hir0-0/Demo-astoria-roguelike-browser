...
- ✅ Overworld **Planície Enferrujada** totalmente jogável: geração procedural, combate (bump-to-attack + 5 skills), inventário/equipamento, inimigos com IA de perseguição, sistema de save/load manual via JSON.
- ✅ Dungeon **Ruínas de Cristal** implementada como **expedição**: o jogador desce até 5 andares gerados proceduralmente (salas conectadas em grafo, com bifurcações e salas opcionais de risco/recompensa), com dificuldade crescente por profundidade, e retorna ao mesmo overworld de onde saiu — o overworld é persistente e nunca é regenerado.
- 🚧 Fora de escopo nesta versão: confronto de chefe roteirizado no andar final, descida além do andar 5, crafting, economia, facções, vila/fazenda ou ranking.
...
- **Subir/sair antecipadamente:** pisar no `stairs_up` do andar 1 restaura `STATE.overworldSnapshot` exatamente como estava (mesmos tiles explorados, entidades, itens, posição do jogador).
