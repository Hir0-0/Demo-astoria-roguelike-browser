// js/core/engine.js — PATCH v0.0.7
//
// MUDANÇAS v0.0.7 (fora da lista original de arquivos a modificar — ver
// justificativa nos comentários [v0.0.7-D] abaixo, necessidade emergente):
//  [v0.0.7-D] startNewGame(): reseta player.statusEffects (sem isso, um status
//             ativo no momento da morte/reinício sobreviveria ao reset, já que
//             o Object.assign de reset não tocava nesse campo).
//  [v0.0.7-D] _isPlayerIncapacitated() + guardas no topo de movePlayer() e
//             playerWait(): freeze/paralyze aplicados ao jogador (v0.0.7-B,
//             combat.js) agora de fato impedem a ação — sem isso, o status
//             ficaria visível na HUD mas sem efeito nenhum sobre o jogador,
//             quebrando a simetria já especificada ("O jogador também pode
//             receber status effects de inimigos"). Quando incapacitado, a
//             tecla pressionada ainda consome o turno (chama _endPlayerTurn
//             diretamente), senão o efeito nunca decrementaria.
//
// MUDANÇAS v0.0.6:
...
class Game {
  ...
  startNewGame() {
    ...
    Object.assign(STATE.player, {
      ...
      // [v0.0.7-D] Limpa status effects de uma run anterior (o Object.assign
      // não reseta campos ausentes deste literal — sem isso, burn/poison/etc.
      // ativos no momento da morte sobreviveriam ao "Nova Partida").
      statusEffects: []
    });
  ...
  // [v0.0.7-D] true se freeze ou paralyze estiver ativo no jogador. Usado por
  // movePlayer/playerWait para impedir a ação e ainda assim consumir o turno
  // (senão o efeito nunca decrementaria — ver processStatusEffects).
  _isPlayerIncapacitated() {
    return !!(getStatusEffect(STATE.player, "freeze") || getStatusEffect(STATE.player, "paralyze"));
  }
  ...
  movePlayer(dx, dy) {
    ...
    if (this._isPlayerIncapacitated()) {
      addLog("Você não consegue agir!", "#85C1E9");
      this._endPlayerTurn();
      return;
    }
