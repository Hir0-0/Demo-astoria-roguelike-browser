// js/ui/hud.js — PATCH v0.0.7
//
// MUDANÇAS v0.0.7:
//  [v0.0.7-F] renderStats() — exibe badges de status effects ativos no jogador
//             (burn/slow/freeze/shock/paralyze/poison), com stacks e turnos
//             restantes quando aplicável. Reaproveita .stat-item/.stat-divider
//             já existentes — nenhuma alteração em css/style.css.
//
// MUDANÇAS v0.0.6:
...
function renderStats() {
  ...
  // [v0.0.7-F] Status effects ativos no jogador (burn/slow/freeze/shock/
  // paralyze/poison — ver STATUS_CONFIG em state.js). Só renderiza a seção
  // quando há algo ativo, para não poluir o painel na maior parte do jogo.
  const STATUS_DISPLAY = {
    burn:     { icon: "🔥", color: ELEMENTS.fire.color,     label: "Queimando" },
    slow:     { icon: "❄",  color: ELEMENTS.ice.color,      label: "Lento" },
    freeze:   { icon: "❄",  color: ELEMENTS.ice.color,      label: "Congelado" },
    shock:    { icon: "⚡", color: ELEMENTS.electric.color, label: "Eletrizado" },
    paralyze: { icon: "⚡", color: ELEMENTS.electric.color, label: "Paralisado" },
    poison:   { icon: "☠",  color: ELEMENTS.poison.color,   label: "Envenenado" }
  };
  const activeStatuses = p.statusEffects || [];
  let statusHtml = "";
  if (activeStatuses.length > 0) {
    const badges = activeStatuses.map(s => {
      const meta = STATUS_DISPLAY[s.type] || { icon: "?", color: "#BDC3C7", label: s.type };
      let extra = "";
      if (s.type === "slow" || s.type === "poison") extra += ` x${s.stacks}`;
      if (typeof s.turnsLeft === "number") extra += ` (${s.turnsLeft}t)`;
      return `<span style="color:${meta.color}" title="${meta.label}${extra}">${meta.icon}${extra}</span>`;
    }).join(" ");
    statusHtml = `
    <div class="stat-divider"></div>
    <div class="stat-item" style="flex-wrap:wrap">
      <span class="stat-label">Status</span>
      <span style="font-size:0.62rem;display:flex;gap:6px;flex-wrap:wrap">${badges}</span>
    </div>`;
  }
