...
function showMainMenu() {
  ...
  ≈≈≈·:·T·······:·:·≈≈≈</pre>
      ...
      <div style="color:#333;font-size:0.6rem;margin-top:10px">v0.0.4 · Planície Enferrujada</div>
    </div>
  `;
}
...
function showVictory() {
  ...
  overlay.style.display = "flex";
  const p = STATE.player;
  // [BUG-09] Esta tela era exibida como "VITÓRIA" ao simplesmente pisar na escada,
  // sugerindo que o jogador venceu o jogo — mas não existe dungeon real implementada
  // ainda (não há generateDungeon()). Renomeada para comunicar corretamente que é o
  // fim do conteúdo desta demo, não uma vitória de fato. gamePhase continua "WIN"
  // internamente; apenas o texto exibido muda.
  overlay.innerHTML = `
    <div class="menu-box">
      <div style="color:#F1C40F;font-size:2.5rem;margin-bottom:4px">✦</div>
      <div class="logo-title" style="color:#F1C40F;font-size:1.6rem">FIM DO DEMO</div>
      <div style="color:#8FBC44;font-size:0.8rem;margin:8px 0;text-align:center">
        ${p.name} explorou toda a Planície Enferrujada e alcançou as Ruínas de Cristal!<br>
        <span style="color:#7F8C8D">A dungeon ainda não existe nesta versão — chega em uma próxima atualização.</span>
      </div>
      <div class="death-stats">
        <div class="death-stat"><span>Nível</span><span style="color:#F0E68C">${p.level}</span></div>
        <div class="death-stat"><span>Turno</span><span style="color:#F0E68C">${STATE.turn}</span></div>
