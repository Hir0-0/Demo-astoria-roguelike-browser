...
// [v0.0.7-E] Pool de inimigos por profundidade. Cada faixa de andar tem uma
// lista de {id, weight} — pesos não precisam somar 1 (são normalizados no
// roll). Substitui o roll uniforme sobre todos os inimigos definidos.
//
// Andares 1–2: só dificuldade baixa (rust_crawler, crystal_shade, toxic_spore).
// Andares 3–4: todos os tipos; steam_golem com frequência baixa (mini-boss).
// Andar 5:     todos os tipos; steam_golem com frequência maior; ice/electric
//              priorizados (pesos mais altos) para desafio tático máximo.
//
// IDs que não existirem em STATE.defs.enemies são filtrados defensivamente em
// pickWeightedEnemyId — um typo aqui não quebra a geração, só reduz o pool.
const ENEMY_POOL_BY_DEPTH = [
  {
    minDepth: 1, maxDepth: 2,
    pool: [
      { id: "rust_crawler",  weight: 0.40 },
      { id: "crystal_shade", weight: 0.35 },
      { id: "toxic_spore",   weight: 0.25 }
    ]
  },
  {
    minDepth: 3, maxDepth: 4,
    pool: [
      { id: "rust_crawler",      weight: 0.15 },
      { id: "crystal_shade",     weight: 0.15 },
      { id: "toxic_spore",       weight: 0.15 },
      { id: "lava_salamander",   weight: 0.20 },
      { id: "ice_elemental",     weight: 0.15 },
      { id: "lightning_conduit", weight: 0.15 },
      { id: "steam_golem",       weight: 0.05 }
    ]
  },
  {
    minDepth: 5, maxDepth: 5,
    pool: [
      { id: "rust_crawler",      weight: 0.05 },
      { id: "crystal_shade",     weight: 0.10 },
      { id: "toxic_spore",       weight: 0.10 },
      { id: "lava_salamander",   weight: 0.15 },
      { id: "ice_elemental",     weight: 0.20 },
      { id: "lightning_conduit", weight: 0.20 },
      { id: "steam_golem",       weight: 0.20 }
    ]
  }
];

// [v0.0.7-E] Retorna a entrada de ENEMY_POOL_BY_DEPTH cuja faixa cobre `depth`.
// Fallback defensivo: se depth exceder MAX_DEPTH por algum motivo, usa a
// última faixa (mais difícil) em vez de quebrar.
function getDepthPoolTier(depth) {
  return ENEMY_POOL_BY_DEPTH.find(t => depth >= t.minDepth && depth <= t.maxDepth)
      || ENEMY_POOL_BY_DEPTH[ENEMY_POOL_BY_DEPTH.length - 1];
}

// [v0.0.7-E] Roll ponderado de um id de inimigo dentro da faixa de `depth`,
// restrito aos ids que de fato existem em `availableIds` (defs.enemies
// carregados). Se a interseção ficar vazia (ex.: defs incompletos em teste),
// cai para roll uniforme sobre availableIds em vez de travar a geração.
function pickWeightedEnemyId(rng, depth, availableIds) {
  const tier = getDepthPoolTier(depth);
  const validPool = tier.pool.filter(p => availableIds.includes(p.id));
  if (validPool.length === 0) {
    return availableIds[Math.floor(rng() * availableIds.length)];
  }
  const totalWeight = validPool.reduce((sum, p) => sum + p.weight, 0);
  let roll = rng() * totalWeight;
  for (const p of validPool) {
    roll -= p.weight;
    if (roll <= 0) return p.id;
  }
  return validPool[validPool.length - 1].id; // fallback de arredondamento
}
...
function spawnDungeonEnemies(tiles, rooms, rng, depth, entranceRoomId, optionalRoomIds, difficultyMult) {
  ...
  for (const room of rooms) {
    ...
    for (let i = 0; i < baseCount; i++) {
      let typeId;
      if (isOptional) {
        // [v0.0.7-E] Sala opcional ainda busca o inimigo mais forte, mas
        // restrito ao pool da profundidade atual — risco/recompensa real
        // sem violar o gate de dificuldade por andar (ex.: sem steam_golem
        // aparecendo em andar 1 só porque a sala era "de risco").
        const tier = getDepthPoolTier(depth);
        const tierIds = tier.pool.map(p => p.id).filter(id => enemyPool.includes(id));
        const candidates = tierIds.length > 0 ? tierIds : enemyPool;
        typeId = candidates.reduce((best, id) => {
          const a = STATE.defs.enemies[id], b = STATE.defs.enemies[best];
          return (a.hp + a.attack * 3) > (b.hp + b.attack * 3) ? id : best;
        }, candidates[0]);
      } else {
        typeId = pickWeightedEnemyId(rng, depth, enemyPool);
      }
